(function () {
    ...
    app.controller('company.login', ['$scope','$stateParams','UserService','$ocLazyLoad','Notification','$window','$timeout','$injector','$http','$log','$state', '$mdDialog','$dialogScope',
        function ($scope, $stateParams, UserService, $ocLazyLoad, Notification, $window,  $timeout, $injector, $http, $log, $state, $mdDialog,$dialogScope) {
          ...
          $scope.registerUser = function(data,valid_data){
            $scope.error = {};
            if(valid_data){
              ...
              UserService.post(data).then(function(data){
               ...
               var userInfo = data.data;
                UserService.getToken(userData.username, userData.password).then(function(response){            
                    var token = response.data.token;
                    getToken(token,userInfo);
                });
                $http.get(
